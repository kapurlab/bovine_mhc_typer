...
def _rp_metrics(run_dir: Path) -> Dict[str, Any]:
    """DRB3 (Class II) call — the production-ready output."""
    d = _rp_json(run_dir, "run_manifest.json")
    drb3 = d.get("drb3") or {}
    return {
        "allele1": drb3.get("allele1") or None,
        "allele2": drb3.get("allele2") or None,
        "zygosity": drb3.get("zygosity") or None,
        "reads": drb3.get("n_reads") or None,
    }
...
_RP_EXPORT_COLUMNS = [("sample", "Sample"), ("status", "Status"),
                      ...
                      ("qc_reasons", "QC notes")] + [("allele1", "DRB3 allele 1"), ("allele2", "DRB3 allele 2"), ("zygosity", "Zygosity"), ("reads", "Reads")] + \
...
def api_project_results_csv(name: str, start: str = Query(""), end: str = Query(""),
    ...
    return Response(content=buf.getvalue(), media_type="text/csv",
                    headers={"Content-Disposition":
                             'attachment; filename="%s_mhc_results.csv"' % name})
...
def api_project_results_xlsx(name: str, start: str = Query(""), end: str = Query(""),
    ...
    return Response(content=buf.getvalue(),
                    ...
                    headers={"Content-Disposition":
                             'attachment; filename="%s_mhc_results.xlsx"' % name})
